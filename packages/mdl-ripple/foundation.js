...
export default class MDLRippleFoundation extends MDLFoundation {
  ...
  constructor(adapter) {
    ...
    this.isSupported_ = this.adapter_.browserSupportsCssVars();
    this.layoutFrame_ = 0;
  ...
  animateUnboundedActivation_() {
    ...
    if (this.activationState_.wasActivatedByPointer) {
      startPoint = getNormalizedEventCoords(
        this.activationState_.activationEvent, this.adapter_.getWindowPageOffset(), this.frame_
      );
    } else {
      startPoint = {
  ...
  animateBoundedDeactivation_(e, isPointerEvent) {
    let startPoint;
    if (isPointerEvent) {
      startPoint = getNormalizedEventCoords(e, this.adapter_.getWindowPageOffset(), this.frame_);
    } else {
      startPoint = {
        left: this.frame_.width / 2,
        top: this.frame_.height / 2
